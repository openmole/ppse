...
This method is efficient to discover possible patterns generated by simulation models but does not provide any information about the (marginal) likelihood of the discovered patterns.
To overcome this limitation we design a new exploration method called Pattern Probability Space Estimation (PPSE).
PPSE discovers the diversty of patterns produced by a simulation model and estimates the marginal likelihood of all the discovered patterns, i.e., the probability of generating the pattern for all possible values of the parameters (also known as model evidence).
...
The motivation is to estimate an approximation of $\pi(y)$, the marginal likelihood of the patterns of the model~\cite{llorente2023marginal}.
