...
def breeding(
  ...
  gmm match 
    case None => Array.fill(lambda)(randomGenomes(genomeDescription, random))
...
def elitism(
  ...
  def keepFirst(i: Vector[I]) = Vector(i.head) // define keepNiches

  val (newGenomes, newPattens) = keepNiches(genomes, patterns, offspringGenomes, offspringPatterns)
  val newHitMap = addHits(offspringPatterns, hitMap)

  def weights(patterns: Array[Array[Int]]) = 
    val w = patterns.map(p => newHitMap.get(p).getOrElse(1))
    w.map(h => 1.0 / h) // wMax - h + 1) //.map(math.pow(_, 2))
  
  // TODO: Consider density in boostraping steps ?
  gmm match 
    case None if newPopulation.size < 10 => (newGenomes, newPatterns, newHitMap, gmm)
    case None =>
      val newGMM =
      ...
      val dilatedGMM = WDFEMGMM.dilate(newGMM, dilation)
      val samplerState = EMPPSE.toSampler(dilatedGMM, random).warmup(warmupSampler)

      (newGenomes, newPatterns, newHitMap, Some((diratedGMM, samplerState))

    case Some((gmm, rejectionState)) =>
       val distribution = WDFEMGMM.toDistribution(gmmValue._1, rng)

       def offSpringDensities =
         (offspringGenomes, offspringPatterns).groupBy { _._2 }.view.mapValues { v => v._1.map(p => 1 / distribution.density(p.toArray))}.toSeq


          def probabilityUpdate(p: (Vector[Int], Seq[Double])) = {
            val (pattern, densities) = p
            val newDensity = densityMap.getOrElse(pattern, 0.0) + densities.sum
            pattern -> newDensity
          }
