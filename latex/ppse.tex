...
def evolution(
  ...
  then
    ...
    val (updatedHitMap, updatedDensityMap, updatedGMM) =
      updateState(
    ...
    evolution(elitedGenomes, elitedPattern, updatedDensityMap, updatedHitMap, updatedGMM orElse gmm, random, generation + 1)
\end{lstlisting}
...
def updateState(
  ...
  warmupSampler: Int,
  minClusterSize: Int,
  random: Random): (HitMap, DensityMap, Option[(GMM, RejectionSamplerState)]) =
  def newHitMap =
  ...
  def newDensityMap =
    def offSpringDensities =
      ...
      groupedGenomes.view.mapValues(v => v.map (p => 1 / p._2).sum).toSeq

    def updatePatternDensity(map: DensityMap, pattern: Array[Int], densitiy: Double): DensityMap =
      map.updatedWith(pattern.toVector)( v => Some(v.getOrElse(0.0) + densitiy))

    offSpringDensities.foldLeft(densityMap) { case (map, (pattern, density)) => updatePatternDensity(map, pattern, density) }

  if genomes.size < minClusterSize * 2
  then (newHitMap, newDensityMap, None)
  else
    def newGMM =
      computeGMM(
        genomes = genomes,
    ...
    (newHitMap, newDensityMap, newGMM)
\end{lstlisting}
...
To estimate the GMM at each step of the algorithm PPSE use the learning EM algorithm exposed in the paper {\color{red} ADD REF}. The function n listing \label{lst:computeGMM} fit a GMM over the genomes already discovered by the algorithms. This genome points are weighed in function of the pattern they produces: the more common is the pattern, the lower is the weight. 

The $computeGMM$ function fist compute the weight for each genome. To achieve that it associate the genome with its corresponding pattern and then get the number of hit that occurred for this pattern until then. From this number of hits the weight is derived given the following function: $weight = 1 - (hit / maxhit)^2$ where $maxhit$ is the maximum of hits among all the patterns. This function attributes high weight to the lowest hits and low weigh for the highest number of hits.
