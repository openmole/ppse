...
In 2015 and we introduced PSE~\cite{cherel2015}.
It has been widely used to discover new insights on simulation models in various application fields such as geography~\cite{cottineau2015modular, raimbault2021modeling}, urban planning~\cite{brasebin20183d}, epidemiology~\cite{edali2020analysis}, ecology~\cite{widyastuti2022assessing} and social-ecological systems~\cite{jahel2023future}.
This method is efficient to list possible pattern but does not provide any information about the plausibility of the discovered pattern.
To overcome this limitation we designed a new exploration method called Pattern Probability Space Estimation (PPSE).
PPSE discovers the diversty of patterns produced by a simulation model and estimate the likelihood that the model produce a given pattern when the input parameters are drawn from a uniform distribution.
...
Let us note $y=model(\theta)$ the results of the simulation of the model using parameters $\theta$.
The motivation is to estimate an approximation of $\pi(y|\theta)$, the posterior distribution of the model.
Nevertheless, in the general cas, the likelihood $p(\theta|y)$ is unknown.

PASSER EN PSEUDO-CODE
...
The signature of a the pattern function provided to the algorithm is exposed in listing \ref{lst:pattern}.
